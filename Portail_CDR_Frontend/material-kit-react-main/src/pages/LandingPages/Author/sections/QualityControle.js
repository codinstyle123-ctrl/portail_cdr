...
function QualityControle() {
  ...
  // -----------------------
  // Add Audit (audit_type = "CONT") States & Handlers
  // -----------------------
  ...
  const handleAddAuthSubmit = async (e) => {
    ...
    try {
      ...
      if (authResponse && authResponse.status === 200) {
        ...
        formData.append("audit_type", "CONT");
        ...
        setAddAuthError("Authentication failed. Check your credentials.");
      }
    } catch (error) {
      console.error("Error in add audit authentication/submit", error);
      setAddAuthError("Authentication error. Please try again.");
    } finally {
      setAddLoading(false);
    }
  };
  ...
  // -----------------------
  // Edit Audit (audit_type = "CONT") States & Handlers
  // -----------------------
  ...
  const handleEditAuthSubmit = async (e) => {
    ...
    try {
      ...
      if (authResponse && authResponse.status === 200) {
        ...
        setEditAuthError("Authentication failed. Check your credentials.");
      }
    } catch (error) {
      console.error("Error updating audit", error);
      setEditAuthError("Authentication error. Please try again.");
    } finally {
  ...
  // -----------------------
  // Delete Audit (audit_type = "CONT") States & Handlers
  // -----------------------
  ...
  const handleDeleteAuthSubmit = async (e) => {
    ...
    try {
      ...
      if (authResponse && authResponse.status === 200) {
        ...
        setDeleteAuthError("Authentication failed. Check your credentials.");
      }
    } catch (error) {
      console.error("Error deleting audit", error);
      setDeleteAuthError("Authentication error. Please try again.");
    } finally {
  ...
  // -----------------------
  // Utility: Create a context for audit images
  // -----------------------
  ...
  // -----------------------
  // Fetch dynamic audits of type "CONT" on mount
  // -----------------------
  useEffect(() => {
    setIsLoading(true);
    const route = "audits/add/catalogues?audit_type=CONT";
    axiosInstance
      .get(route)
      .then((response) => {
  ...
  return (
    <MKBox component="section" py={2}>
      ...
      {/* Header with Title, File Input & "Ajouter Audit" button */}
      <div
        style={{
      ...
      <Container>
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} lg={4}>
            <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
              <FilledInfoCard
                ...
                icon=""
                title="LP71.2 Migrations des sites distants vers CSG : Cas Radio"
                description=""
                action={{ type: "internal", route: "/controle/lp71_2", label: "" }}
              />
            </div>
          </Grid>
          <Grid item xs={12} lg={4}>
            <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
              <FilledInfoCard
                ...
                color="rgba(242,242,242,1)"
                icon=""
                title="LP71.2 Migrations des sites distants vers CSG : Cas CSG"
                description=""
                action={{ type: "internal", route: "/controle/lp71_2_csg", label: "" }}
              />
            </div>
          </Grid>
          <Grid item xs={12} lg={4}>
            <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
              <FilledInfoCard
                ...
                color="rgba(242,242,242,1)"
                icon=""
                title="LP 31.8 Déploiement CSG IMES"
                description={
                  <span style={{ backgroundColor: "transparent", color: "#DAA520", fontWeight: "bold" }}>
                    (En cours Dev)
                  </span>
                }
          ...
          <Grid item xs={12} lg={4}>
            <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
              <FilledInfoCard
                ...
                color="rgba(242,242,242,1)"
                icon=""
                title="LP 36 Démontage Trans/radio"
                description={
                  <span style={{ backgroundColor: "transparent", color: "#DAA520", fontWeight: "bold" }}>
                    (En cours Dev)
                  </span>
          ...
          {/* Render Custom Audits (Dynamic Audits) */}
          {customAudits &&
      ...
      {/* --------------------------
          Dialog to display clicked image
      -------------------------- */}
      <Dialog
        ...
        maxWidth="xl"
      >
        <DialogTitle>Audit Image</DialogTitle>
        <DialogContent
          style={{ display: "flex", justifyContent: "center", alignItems: "center", padding: 0, height: "100vh" }}
        >
          {selectedAuditImage && (
            <img
      ...
      {/* --------------------------
          Add Audit Dialog with Authentication for Quality Controle
      -------------------------- */}
      ...
      {/* --------------------------
          Edit Audit Dialog with Authentication & Preview for Quality Controle
      -------------------------- */}
      ...
      {/* --------------------------
          Delete Audit Dialog with Authentication for Quality Controle
      -------------------------- */}
...
export default QualityControle;
