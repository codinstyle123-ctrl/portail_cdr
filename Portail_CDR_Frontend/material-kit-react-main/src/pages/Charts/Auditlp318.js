...
const Page = () => {
  ...
  useEffect(() => {
    axiosInstance
      .get("/audits/api/auditcsgctr/tauxdecoherence")
  ...
  useEffect(() => {
    axiosInstance
      .get("/audits/api/auditcsgctr/repartitioncsg")
      .then((response) => setRepcsg(response.data))
      .catch((error) => console.error("Error fetching data:", error));
  }, []);
  useEffect(() => {
    axiosInstance
      .get("/audits/api/auditcsgctr/repartionincoherence")
  ...
  useEffect(() => {
    axiosInstance
      .get("/audits/api/auditcsgctr/evolutiontauxdecoherence")
      ...
      >
        <div style={{ backgroundColor: "#0055a4" }}>
          <MKTypography variant="h1" align="center" mb={0} mt={1} sticky color="white">
            Audit CSG CTR VIA FH <GeneralFileInput csrfTokenUrl="/fluxoa/api/get_csrf_token/"
