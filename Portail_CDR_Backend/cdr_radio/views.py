...
class UploadExcelView(View):
  def post(self, request):
    ...
    try:
        ...
        start_of_month = first_row_date.replace(day=1)
        end_of_month = (start_of_month + timedelta(days=31)).replace(day=1) - timedelta(days=1)

	# Check if any records with the same month exist in the database
        existing_dates = Fluxoa.objects.filter(
             DATE__range=[start_of_month, end_of_month]
        ).exists()

        if existing_dates:
            return JsonResponse({'success': False, 'error': 'A date in the same month already exists in the database.'}, status=400)
...
class GroupByDateAndStatusView(View):
    def get(self, request, *args, **kwargs):
        try:
            # Get the latest date
            latest_date = Fluxoa.objects.aggregate(latest_date=Max('DATE'))['latest_date']

            if not latest_date:
                return JsonResponse({'success': False, 'error': 'No data found in the DATE column.'}, status=400)

            # Calculate the start and end of the latest month
            start_of_latest_month = latest_date.replace(day=1)
            end_of_latest_month = (start_of_latest_month + timedelta(days=31)).replace(day=1) - timedelta(days=1)

            # Calculate the start of the second latest month
            start_of_second_latest_month = (start_of_latest_month - timedelta(days=1)).replace(day=1)

            # Query to group data by the latest and second latest month
            data = (
                Fluxoa.objects
                .filter(STATUS__in=['cellule normale', 'Cellule en RR ou BDR uniquement', 'Demonte mais present dans BDE'])
                .annotate(month=TruncMonth('DATE'))
                .filter(month__in=[start_of_latest_month, start_of_second_latest_month])
                .values('month', 'STATUS')
            ...
            result_data = {}
            for entry in data:
                month_start = entry['month'].strftime('%Y-%m-%d')  # Format the month start date
                status = entry['STATUS']
                count = entry['count']

                if month_start not in result_data:
                    result_data[month_start] = {}

                result_data[month_start][status] = count

            return JsonResponse({'success': True, **result_data})

        except Exception as e:
            print(f"Error grouping by month and filtering by status: {str(e)}")
...
class TableDataView(View):
    def get(self, request, *args, **kwargs):
        try:
            # Get the latest month in the DATE column
            latest_date = Fluxoa.objects.aggregate(latest_date=Max('DATE'))['latest_date']
            ...
            # Query to get data for the latest month
            data = (
                Fluxoa.objects
                .filter(
                ...
                .annotate(month=TruncMonth('DATE'))
                .filter(month=start_of_latest_month)
                .values('month', 'STATUS', 'ETAT_EXPLOITATION_CRS')
                .annotate(count=Count('ID'))
                .order_by('month')
            )
            ...
            result_data = {}
            for entry in data:
                # Format the month start date as YYYY-MM
                month_start = entry['month'].strftime('%Y-%m')
                status = entry['STATUS']
                ...
                if status not in result_data[month_start]:
                    result_data[month_start][status] = {}

                result_data[month_start][status][etat_exploitation_crs] = count
            ...
            for month_data in result_data.values():
                for status, etat_exploitation_crs in possible_combinations:
                    if status not in month_data:
                        month_data[status] = {}
                    if etat_exploitation_crs not in month_data[status]:
                        month_data[status][etat_exploitation_crs] = 0

            return JsonResponse({'success': True, 'data': result_data})
        except Exception as e:
            print(f"Error grouping by the latest month and filtering by status: {str(e)}")
            return JsonResponse({'success': False, 'error': str(e)})
...
"""
                ...
                smtp_port = 25
                to_email = "elie.saad.ext@nokia.com" 
