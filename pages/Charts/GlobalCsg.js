...
const GlobalCsg = () => {
  ...
  useEffect(() => {
    axiosInstance
      .get("/audits/api/globalcsg/tauxdecoherence")
  ...
  useEffect(() => {
    axiosInstance
      .get("/audits/api/globalcsg/repatitionresponsabilite") 
      .then((response) => setRepcsg(response.data))
      .catch((error) => console.error("Error fetching data:", error));
  }, []);
  useEffect(() => {
    axiosInstance
      .get("/audits/api/globalcsg/repartitionincoherence")
  ...
  useEffect(() => {
    axiosInstance
      .get("/audits/api/globalcsg/evolutiontauxdecoherence")
      .then((response) => setData(response.data))
      .catch((error) => console.error("Error fetching data:", error));
  }, []);

  const chartSeries = Object.values(repcsg);
  const labels = Object.keys(repcsg);
  const chartSeriesnok = Object.values(nokData);
  const labelsnok = Object.keys(nokData);
  useEffect(() => {
    axiosInstance
      .get("/audits/api/globalcsg")
      .then((response) => {
        const groupedData = groupDataByMonth(response.data.aggregated_data);
...
const columnsCSGCTRFH = ["id","N_CSG","Constructeur_CSG","Architecture","Connexion","Equipment_parent","Equipement_Parent_BDR","Delta_Equipement_parent","Port_CSG","Port_CSG_BDR","Delta_Port_CSG","Etat_port_CSG","LAG_CSG","LAG_CSG_BDR","Delta_LAG_CSG","Loopback0_CSG","Loopback0_CSG_BDR","Delta_Loopback0_CSG","Loopback1_CSG","Loopback1_CSG_BDR","Delta_Loopback1_CSG","IP_Interco_CSG","IP_Interco_CSG_BDR","Delta_IP_Interco_CSG","Vlan_Interco_CSG","VLAN_Interco_CSG_BDR","Delta_Vlan_Trafic_CSG","IP_SUP_CSG","IP_Interco_SUP_CSG_BDR","Delta_IP_Interco_SUP_CSG_BDR","VLAN_SUP_CSG","VLAN_Interco_SUP_CSG_BDR","Delta_VLAN_Interco_SUP_CSG_BDR","PORT_Parent","PORT_CTR_BDR","Delta_Port_CTR","LAG_Parent","Etat_Port_Parent","LAG_CTR_BDR","Delta_LAG_CTR","IP_Interco_Parent","IP_Interco_CTR_BDR","Delta_IP_Interco_CTR_BDR","Vlan_Interco_Parent","VLAN_Interco_CTR_BDR","Delta_VLAN_Interco_CTR_BDR","IP_SUP_Parent","IP_Interco_SUP_CTR_BDR","Delta_IP_Interco_SUP_CTR_BDR","VLAN_SUP_Parent","VLAN_SUP_CTR_BDR","Delta_VLAN_SUP_CTR_BDR","IP_RADIO","IP_Radio_BDR","Delta_IP_Radio_BDR","PORT_RADIO","Port_Radio_BDR","Delta_Port_Radio_BDR","LAG_RADIO","LAG_Radio_BDR","Delta_LAG_Radio_BDR","VLAN_RADIO","VLAN_Radio_BDR","Delta_VLAN_Radio_BDR","ETAT_BDR_BDE","Lien_IP_Interco","Routage_Lien_IP","Lien_IP_SUP","Routage_Lien_IP_SUP","Lien_AGG","Routage_Lien_AGG","Lien_PT","Routage_Lien_PT","Lien_ZT_CSG_FHT","LAG","VLAN","Adresse_IP_Interco","Adresse_IP_SUP","Adresse_IP_Loopback0_1","Adresse_IP_Radio","Modelisation_des_Liens","Port","Routage_des_Liens","ETAT_CSG","Date_column","Usine_Prod"];
  return (
    <>
      ...
      <div
        style={{
          backgroundImage: `url(${bgImage})`, // The path to the background image that was imported.
      ...
      >
        ...
        <Container maxWidth="xl">
          <Grid container spacing={3}>
            ...
            {chartSeriesnok && chartSeriesnok.length > 0 && labelsnok && labelsnok.length > 0 ? (
                <OverviewTraffic
                  chartSeries={chartSeriesnok}
                  labels={labelsnok}
                  sx={{ height: "100%" }}
                />
              ) : (
